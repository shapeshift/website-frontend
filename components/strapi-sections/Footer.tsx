...
export default function Footer({data}: {data: TFooterSection}): ReactNode | null {
	...
	return (
		<section
			className={
				'relative mb-40 h-[100px] w-full overflow-hidden rounded-2xl sm:h-[200px] md:h-[300px] lg:h-[504px]'
			}>
			{data?.imageBg?.url && (
				<Image
			...
			<div className={'absolute inset-0 z-10 flex w-full flex-col items-center justify-center gap-4 px-4'}>
				...
				<div
					className={
						'mb-[60px] mt-4 max-w-[800px] text-center text-[36px] font-normal leading-[36px] text-white md:text-[48px] md:leading-[48px] lg:text-h1 lg:leading-[72px]'
					}>
					{data?.title}
				</div>
				<Button
