 ...
 ************************************************************************************************/
export async function fetchProtocolBySlug(slug: string): Promise<TSupportedProtocolData | null> {
	try {
		const response = await fetch(
		...
		console.error(
			`Error fetching protocol data for slug "${slug}":`,
			error instanceof Error ? error.message : String(error)
		);
		return null;
	}
...
/************************************************************************************************
 * Fetch Chain data by slug
 *
 * @param slug - Slug of the chain to fetch
 * @returns Promise resolving to chain data or null if not found
 ************************************************************************************************/
export async function fetchChainBySlug(slug: string): Promise<TSupportedChainData | null> {
	try {
		const response = await fetch(
		...
		if (!response.ok) {
			console.error(`Failed to fetch chain data for slug "${slug}": ${response.status}`);
			return null;
		}
		...
		console.error(
			`Error fetching chain data for slug "${slug}":`,
			error instanceof Error ? error.message : String(error)
		);
		return null;
	}
 ...
 ************************************************************************************************/
export async function fetchWalletBySlug(slug: string): Promise<TSupportedWalletData | null> {
	try {
		const response = await fetch(
		...
		console.error(
			`Error fetching wallet data for slug "${slug}":`,
			error instanceof Error ? error.message : String(error)
		);
		return null;
	}
